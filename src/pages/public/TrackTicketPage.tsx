...
import { Search, Loader2, AlertCircle, CalendarClock, User, Download, Share2 } from 'lucide-react';
...
// Struktur response disesuaikan dengan update backend (Multi-Tiket)
...
export default function TrackTicketPage() {
  ...
  const fetchTicket = async (order_id: string, email: string, attempt: number) => {
    ...
    try {
      const res = await api.get('/api/tickets/track', {
      ...
      if (expectedStatus === 'settlement' && payload.status === 'pending' && attempt <= 3) {
        ...
        setTimeout(() => {
          fetchTicket(order_id, email, attempt + 1);
        }, 3000); 
        return;
      }
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Tiket SMILE FEST 2026',
          text: 'Ini e-ticket SMILE FEST 2026 milik saya!',
          url: url
        });
      } catch (err) {
      ...
      <div className="w-full max-w-2xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-6 rounded-2xl shadow-soft border border-stone-100 space-y-4 mb-8 max-w-md mx-auto">
          <div>
            <input
              ...
              className={`w-full px-4 py-3 bg-stone-50 rounded-xl border transition-colors ${errors.order_id ? 'border-ringkai-danger' : 'border-stone-200 focus:border-ringkai-olive'}`}
            />
          ...
          <div>
            <input
              ...
              className={`w-full px-4 py-3 bg-stone-50 rounded-xl border transition-colors ${errors.email ? 'border-ringkai-danger' : 'border-stone-200 focus:border-ringkai-olive'}`}
            />
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-stone-800 text-white py-3 rounded-xl font-medium flex items-center justify-center gap-2 hover:bg-stone-700 transition-all disabled:opacity-70"
          >
          ...
          {pollingMessage && (
            <p className="text-xs text-center text-amber-600 animate-pulse mt-2">{pollingMessage}</p>
          )}
        </form>
        ...
        {ticketData && (
          <div className="space-y-6 animate-in slide-in-from-bottom-4 fade-in duration-500">
            ...
            {/* Render Tiket (Multi-Tiket Grid) */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {ticketData.status === 'settlement' && ticketData.tickets ? (
                ticketData.tickets.map((ticket, index) => (
                  <div key={ticket.id} className="bg-white border border-stone-200 rounded-3xl p-8 text-center shadow-soft relative overflow-hidden flex flex-col items-center">
                    ...
                    <button 
                      ...
                      className="w-full py-3 bg-stone-100 text-stone-700 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 hover:bg-stone-200 transition-colors"
                    >
                ...
                <div className="col-span-1 md:col-span-2 bg-white border border-stone-200 rounded-3xl p-12 flex flex-col items-center justify-center shadow-soft text-stone-400">
                  <CalendarClock className="w-16 h-16 mb-4 opacity-50" />
                  <p className="font-medium text-center">Selesaikan pembayaran terlebih dahulu<br/>untuk memunculkan E-Ticket Anda.</p>
                </div>
              )}
            </div>
